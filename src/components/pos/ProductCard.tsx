...
export const ProductCard = memo(function ProductCard({ currency, onAdd, product }: ProductCardProps) {
  ...
  return (
    <article className={cn("group flex h-full flex-col overflow-hidden rounded-2xl border bg-card shadow-sm transition duration-200", canAdd ? "border-border hover:-translate-y-0.5 hover:border-primary hover:shadow-lg" : "border-stone-200 opacity-65")}>
      ...
      <div className="flex flex-1 flex-col p-3">
        ...
        <h3 className="mt-1 line-clamp-2 min-h-10 text-sm font-black leading-5 text-secondary">{product.name}</h3>
