...
export const ProductCard = memo(function ProductCard({ currency, onAdd, product }: ProductCardProps) {
  return (
    <article className={cn("group overflow-hidden rounded-2xl border bg-card shadow-sm transition duration-200", product.available ? "border-border hover:-translate-y-0.5 hover:border-primary hover:shadow-lg" : "border-stone-200 opacity-65")}>
      <div className="relative h-28 overflow-hidden sm:h-32">
        <ProductImage product={product} />
        <span className={cn("absolute right-2 top-2 rounded-full px-2 py-1 text-[0.62rem] font-black uppercase tracking-wide backdrop-blur", product.available ? "bg-success/90 text-white" : "bg-secondary/85 text-white")}>
      ...
      <div className="p-3">
        ...
        <p className="mt-1 text-base font-black text-secondary">{formatMoney(product.price, currency)}</p>
        <button
          ...
          type="button"
        >
          {product.available ? <Plus aria-hidden="true" className="size-5" /> : <ShoppingBag aria-hidden="true" className="size-4" />}
