...
export const ProductGrid = memo(function ProductGrid({ currency, onAdd, products }: ProductGridProps) {
  ...
  return <section aria-label="Menu products" className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">{products.map((product) => <ProductCard currency={currency} key={product.id} onAdd={onAdd} product={product} />)}</section>;
});
