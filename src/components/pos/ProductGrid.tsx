...
export const ProductGrid = memo(function ProductGrid({ currency, onAdd, products }: ProductGridProps) {
  ...
  return <section aria-label="Menu products" className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-2 2xl:grid-cols-3">{products.map((product) => <ProductCard currency={currency} key={product.id} onAdd={onAdd} product={product} />)}</section>;
});
