        ...
        <div className="space-y-5">
          <section className="dashboard-card rounded-2xl border border-border bg-card p-5 sm:p-6">
            ...
            <div className="overflow-x-auto rounded-xl border border-border dashboard-scrollbar">
              <table className="w-full min-w-[600px] text-left"><thead className="bg-secondary text-xs uppercase tracking-wide text-white/70"><tr><th className="px-4 py-3">Item</th><th className="px-4 py-3 text-center">Quantity</th><th className="px-4 py-3 text-right">Unit price</th><th className="px-4 py-3 text-right">Line total</th></tr></thead><tbody className="divide-y divide-border">{order.items.map((item) => <tr key={item.id}><td className="px-4 py-3"><p className="text-sm font-black text-secondary">{item.name}{item.variantName ? ` — ${item.variantName}` : ""}</p>{item.notes && <p className="text-xs text-muted-foreground">{item.notes}</p>}</td><td className="px-4 py-3 text-center text-sm font-bold">{item.quantity}</td><td className="px-4 py-3 text-right text-sm font-semibold">{money(item.unitPrice)}</td><td className="px-4 py-3 text-right text-sm font-black text-secondary">{money(item.totalPrice)}</td></tr>)}</tbody></table>
            </div>
