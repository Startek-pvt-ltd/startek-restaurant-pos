...
export function ReceiptPreviewToolbar({ autoPrint, copies, orderId, printerName }: { autoPrint: boolean; copies: number; orderId: string; printerName: string }) {
  ...
  useEffect(() => { if (!autoPrint || opened.current) return; opened.current = true; let cancelled = false; void printReceiptWhenReady().catch((error) => { if (!cancelled) toast.error(error instanceof Error ? error.message : "Receipt could not be prepared for printing."); }); return () => { cancelled = true; }; }, [autoPrint]);
