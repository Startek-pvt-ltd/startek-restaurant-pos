...
export function SalesChart({ data }: { data: Array<{ label: string; value: string }> }) {
  ...
  const hasSales = weeklySales.some((item) => item.sales > 0);
  return (
    <article className="dashboard-card dashboard-fade-in rounded-2xl border border-border/80 bg-card p-5 sm:p-6">
      ...
      {!hasSales ? <DashboardEmptyState icon={ChartNoAxesCombined} message="No completed sales have been recorded this week." /> : <div aria-label="Weekly sales chart" className="h-72 w-full" role="img">
